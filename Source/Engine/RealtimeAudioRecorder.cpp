...
namespace
{
constexpr double ringBufferSeconds = 2.0;
constexpr double initialRecordedSeconds = 60.0;
constexpr std::size_t minimumRingSamples = 16384;
}
...
void RealtimeAudioRecorder::start(const double sampleRate)
{
    ...
    auto ringSamples = static_cast<std::size_t>(
        std::ceil(recordingSampleRate * 2.0 * ringBufferSeconds));
    ringSamples = std::max(minimumRingSamples, ringSamples + (ringSamples & 1U));
    ringBuffer.assign(ringSamples, 0.0f);
    recordedInterleaved.clear();
    recordedInterleaved.reserve(static_cast<std::size_t>(
        recordingSampleRate * 2.0 * initialRecordedSeconds));
                                 ...
                                 const int numFrames) noexcept
{
    ...
    const auto requested = static_cast<std::size_t>(numFrames) * 2;
    const auto used = write - read;
    auto writable = std::min(requested, capacity - std::min(capacity, used));
    writable &= ~std::size_t { 1 };
    auto ringIndex = write % capacity;
    const auto writableFrames = writable / 2;
    ...
    writePosition.store(write + writable, std::memory_order_release);
    if (writableFrames < static_cast<std::size_t>(numFrames))
        droppedFrames.fetch_add(static_cast<std::uint64_t>(numFrames)
                                    - writableFrames,
...
std::size_t RealtimeAudioRecorder::drainAvailable()
{
    ...
    const auto first = std::min(available, capacity - ringIndex);
    recordedInterleaved.insert(recordedInterleaved.end(),
        ringBuffer.begin() + static_cast<std::ptrdiff_t>(ringIndex),
        ringBuffer.begin() + static_cast<std::ptrdiff_t>(ringIndex + first));
    const auto second = available - first;
    if (second > 0)
        recordedInterleaved.insert(recordedInterleaved.end(),
                                   ringBuffer.begin(),
                                   ringBuffer.begin()
    ...
    recordedSampleCount.store(recordedInterleaved.size(),
